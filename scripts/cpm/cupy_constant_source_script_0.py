...
from pyapm.tools.cupy import cupy_ctsf, cupy_ctsp, cupy_ctsv
from pyapm.tools.mesh import point_mesh_xy, point_mesh_yz, point_mesh_zx
from pyapm.tools.numpy import numpy_ctsf, numpy_ctsp, numpy_ctsv
...
start = perf_counter()
flwn = numpy_ctsf(pnts, grda, grdb, grdc, tol = 1e-12, cond = cond)
finish = perf_counter()
elapsedn = finish - start
print(f'Numpy time elapsed is {elapsedn:.6f} seconds.')

start = perf_counter()
flwc = cupy_ctsf(pnts, grda, grdb, grdc, tol = 1e-12, cond = cond)
finish = perf_counter()
...
_ = axd.set_title('3D Source Panel Velocity Potential - Numpy')

axp, cfp = point_contourf_xy(pnts, flwc.phi, levels=cfd.levels)
_ = axp.set_title('3D Source Panel Velocity Potential - Cupy')

# Doublet Velocity in X
axd, cfd = point_contourf_xy(pnts, flwn.vel.x)
_ = axd.set_title('3D Source Panel Velocity in X - Numpy')

axp, cfp = point_contourf_xy(pnts, flwc.vel.x, levels=cfd.levels)
_ = axp.set_title('3D Source Panel Velocity in X - Cupy')

# Doublet Velocity in Y
axd, cfd = point_contourf_xy(pnts, flwn.vel.y)
_ = axd.set_title('3D Source Panel Velocity in Y - Numpy')

axp, cfp = point_contourf_xy(pnts, flwc.vel.y, levels=cfd.levels)
_ = axp.set_title('3D Source Panel Velocity in Y - Cupy')

# Doublet Velocity in Z
axd, cfd = point_contourf_xy(pnts, flwn.vel.z)
_ = axd.set_title('3D Source Panel Velocity in Z - Numpy')

axp, cfp = point_contourf_xy(pnts, flwc.vel.z, levels=cfd.levels)
_ = axp.set_title('3D Source Panel Velocity in Z - Cupy')

#%%
# Check Components
phic = cupy_ctsp(pnts, grda, grdb, grdc, cond = cond)
velc = cupy_ctsv(pnts, grda, grdb, grdc, cond = cond)
...
phin = numpy_ctsp(pnts, grda, grdb, grdc, cond = cond)
veln = numpy_ctsv(pnts, grda, grdb, grdc, cond = cond)
...
axp, cfp = point_contourf_xz(pnts, flwc.phi, levels=cfd.levels)
_ = axp.set_title('3D Source Panel Velocity Potential - Cupy')
...
axp, cfp = point_contourf_xz(pnts, flwc.vel.x, levels=cfd.levels)
_ = axp.set_title('3D Source Panel Velocity in X - Cupy')
...
axp, cfp = point_contourf_xz(pnts, flwc.vel.y, levels=cfd.levels)
_ = axp.set_title('3D Source Panel Velocity in Y - Cupy')
...
axp, cfp = point_contourf_xz(pnts, flwc.vel.z, levels=cfd.levels)
_ = axp.set_title('3D Source Panel Velocity in Z - Cupy')
...
axp, cfp = point_contourf_yz(pnts, flwc.phi, levels=cfd.levels)
_ = axp.set_title('3D Source Panel Velocity Potential - Cupy')
...
axp, cfp = point_contourf_yz(pnts, flwc.vel.x, levels=cfd.levels)
_ = axp.set_title('3D Source Panel Velocity in X - Cupy')
...
axp, cfp = point_contourf_yz(pnts, flwc.vel.y, levels=cfd.levels)
_ = axp.set_title('3D Source Panel Velocity in Y - Cupy')
...
axp, cfp = point_contourf_yz(pnts, flwc.vel.z, levels=cfd.levels)
_ = axp.set_title('3D Source Panel Velocity in Z - Cupy')
